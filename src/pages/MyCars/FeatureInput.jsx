...
function FeatureInput({ features, setFeatures, btnLoading, id }) {
  ...
  const handleFeatures = (e) => {
    ...
    if (e.key === "Enter" && value !== "") {
      ...
      if (!features.includes(value)) {
        setFeatures(prev => ({ ...prev, [id]: [...features, value] }))
        e.target.value = ""
      } else {
...

        {
          features.map(
            (itm, idx) => <div className={`h-max inline-flex items-center gap-1 bg-emerald-800 px-2 rounded-md mr-2 mt-2 ${btnLoading ? "pointer-events-none" : null}`} key={idx}>
              <span>{itm}</span>
              <IoCloseCircleSharp className={`text-xl ${btnLoading ? "cursor-not-allowed" : "cursor-pointer"}`} onClick={() => setFeatures(prev => {
                const filteredArr = prev[id].filter(feature => feature !== itm)
                return { ...prev, [id]: [...filteredArr] }
              })} />
            </div>
          )
        }
